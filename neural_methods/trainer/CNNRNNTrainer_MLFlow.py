...
class CNNRNNTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        self.best_epoch = 0
        # self.best_run_artifact_path = None  # [MLflow] 记录最佳权重文件的路径
    ...
    def train(self, data_loader):
        ...
        # === 开始一个 MLflow run：以当前 lr 命名 ===
        with mlflow.start_run(run_name=f"lr={float(self.config.TRAIN.LR)}"):
            # 记录关键超参（你想多记都可以）
            mlflow.log_param("lr", float(self.config.TRAIN.LR))
            mlflow.log_param("epochs", int(self.config.TRAIN.EPOCHS))
            mlflow.log_param("batch_size", int(self.config.TRAIN.BATCH_SIZE))
            mlflow.log_param("frame_depth", int(self.frame_depth))
            mlflow.log_param("optimizer", "AdamW")
            mlflow.log_param("scheduler", "OneCycleLR")
            mlflow.set_tag("model", "CNNRNN")
            mlflow.set_tag("toolbox_mode", self.config.TOOLBOX_MODE)
            ...
            for epoch in range(self.max_epoch_num):
                ...
                # epoch 末尾统计
                epoch_train_loss = float(np.mean(train_loss))
                ...
                mlflow.log_metric("lr", cur_lr, step=epoch)

                self.save_model(epoch)

                # 验证并写入
                if not self.config.TEST.USE_LAST_EPOCH:
                    valid_loss = float(self.valid(data_loader))
                    ...
                    if self.min_valid_loss is None or valid_loss < self.min_valid_loss:
                        ...
                        # 同步上传最佳 checkpoint，方便回滚/部署
                        best_path = os.path.join(self.model_dir, f"{self.model_file_name}_Epoch{epoch}.pth")
                        if os.path.exists(best_path):
                            mlflow.log_artifact(best_path, artifact_path="checkpoints/best")

            if not self.config.TEST.USE_LAST_EPOCH:
                print(f"Best trained epoch: {self.best_epoch}, Min val loss: {self.min_valid_loss}")
                mlflow.log_param("best_epoch", int(self.best_epoch))
                mlflow.log_metric("best_val_loss", float(self.min_valid_loss))
